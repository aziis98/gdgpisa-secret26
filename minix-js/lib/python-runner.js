 ...
 *
 * Provides a unified interface to run Python code in Pyodide with:
 * - Interactive terminal I/O (input/output)
 * - Virtual filesystem access (read/write)
 * - Async support for input() calls
 */

/**
 * Run Python code in a virtualized Pyodide environment
 *
 * @param {string} code - Python code to execute
 * @param {object} context - Execution context
 * @param {object} context.py - Pyodide instance
 * @param {object} context.vfs - Virtual filesystem with read(path) and write(path, content) methods
 * @param {object} context.stdoutWrite - Callback function for writing output: (string) => void
 * @param {object} context.readLine - Async callback function for reading input lines
 * @param {object} context.scriptName - Script name for sys.argv[0]
 * @param {string[]} context.scriptArgs - Command line arguments for sys.argv[1:]
 * @param {string} context.stdin - Input string (lines separated by \n) for non-interactive mode
...
export async function runPythonAsync(code, context = {}) {
    ...
    try {
        ...
        py.setStdout({
            raw: byteCode => {
                console.log("[stdout] Received byte code:", byteCode, stdoutWrite)
        ...
        py.setStderr({
            raw: byteCode => {
                console.log("[stderr] Received byte code:", byteCode, stdoutWrite)
                const char = String.fromCharCode(byteCode)
                if (char === "\n") {
                    if (stdoutWrite) stdoutWrite("\n")
                    outputBuffer += char
