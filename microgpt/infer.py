...
def generate(initial_tokens, on_token=None):
    """Generate text starting from initial tokens.

    Args:
        initial_tokens: List of token IDs to start from
        on_token: Optional callback function called for each generated character
    """
...
if args.sample:
    ...
    while True:
        ...
        if not user_input:
            ch = random.choice("hdsabg")
            user_input = str(ch)

        # Tokenize user input
        invalid_chars = [ch for ch in user_input if ch not in uchars]
        if invalid_chars:
            print(f"Error: Invalid character(s) not in vocabulary: {', '.join(set(invalid_chars))}\n")
            continue

        initial_tokens = [BOS] + [uchars.index(ch) for ch in user_input]
        print(user_input, end="", flush=True)
        generate(initial_tokens, on_token=lambda ch: (print(ch, end="", flush=True), time.sleep(0.01)))
        print("\n")
