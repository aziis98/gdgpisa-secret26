...
if not os.path.exists("vocab.json") or not os.path.exists("model_weights.json"):
    raise FileNotFoundError(
        "Run train.py first to generate vocab.json and model_weights.json"
    )
...
state_dict = {
    k: [[Value(val) for val in row] for row in mat] for k, mat in raw_state_dict.items()
}
...
def gpt(token_id, pos_id, keys, values):
    ...
    for li in range(n_layer):
        ...
        for h in range(n_head):
            ...
            attn_logits = [
                sum((q_h[j] * k_h[t][j] for j in range(head_dim)), Value(0))
                / head_dim**0.5
...
def generate(initial_tokens):
    """Generate text starting from initial tokens as a generator.
    ...
    # Process all initial tokens except the last one to build up keys/values
    pos_id = 0
    for token in initial_tokens[:-1]:
        logits = gpt(token, pos_id, keys, values)
        pos_id += 1

    # Now generate new tokens, starting from the last initial token
...
parser.add_argument(
    ...
    type=int,
    metavar="COUNT",
    help="Generate COUNT random samples instead of REPL",
)
parser.add_argument(
    "-p", "--prefix", type=str, default="", help="Prefix to use when generating samples"
)
parser.add_argument(
    "-t",
    "--temperature",
...
if args.sample:
    ...
    if args.prefix:
        ...
        if invalid_chars:
            m = ", ".join(set(invalid_chars))
            print(f"Error: Invalid characters in prefix: {m}")
            sys.exit(1)
    ...
    while True:
        ...
        if invalid_chars:
            m = ", ".join(set(invalid_chars))
            print(f"Error: Invalid characters not in vocabulary: {m}\n")
            continue

        initial_tokens = [BOS] + [uchars.index(ch) for ch in user_input]
        print(user_input, end="", flush=True)
        for ch in generate(initial_tokens):
